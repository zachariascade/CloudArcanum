...
The standalone repo does not include battle-simulation folders, so there is no need to navigate around cross-product areas.

## Good Defaults

- treat `src/domain/` as shared schema, not product behavior
- prefer Arcanum-owned tests when verifying Arcanum changes
- prefer product-local scripts such as `dev`, `validate`, and `arcanum:process:*`
