...
Cloud Arcanum does not own combat simulation, session state, replay logic, or battle rules.
...
Cloud Arcanum does not carry battle simulation or replay modules, so any future integration should be treated as an external boundary rather than a local import.

## Runtime Shape

Typical local flow:

- `dev:api` runs the catalog API
- `dev:web` runs the Arcanum frontend
